...
WHERE row_num > 1;
-- Assigns a row number to each duplicate row (based on those listed columns), restarting from 1 for each identical group.
...
SELECT * 
FROM layoffs_staging_2 
WHERE company IS NULL OR location IS NULL;
...
SELECT * FROM layoffs_staging_2;
-- Data Cleaning/Preparation done. Next up is Exploratory Data Analysis 